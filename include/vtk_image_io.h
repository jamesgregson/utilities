#ifndef VTR_LOAD_SAVE_H
#define VTR_LOAD_SAVE_H
...
inline real3 world_to_grid( const real *aabb, const index *dim, const real3 &pos ){
    return real3( real(dim[0])*(pos[0]-aabb[0])/(aabb[1]-aabb[0]), real(dim[1])*(pos[1]-aabb[2])/(aabb[3]-aabb[2]), real(dim[2])*(pos[2]-aabb[4])/(aabb[5]-aabb[4]) );
}

template< typename real, typename index, typename real3 >
inline real3 grid_to_world( const real *aabb, const index *dim, const real3 &pos ){
    return real3( aabb[0]+real(pos[0])*(aabb[1]-aabb[0])/real(dim[0]), aabb[2]+real(pos[1])*(aabb[3]-aabb[2])/real(dim[1]), aabb[4]+real(pos[2])*(aabb[5]-aabb[4])/real(dim[2]) );
}
*/
